...
namespace IDMP_ros
{
    ...
    public:
        ...
        EMatrixX kernel_sparse_deriv1_3D(EMatrixX const& x1, float scale_param, float sigx);
        //Testing
        void testSinglePoint_new(const EVectorX &xt, double &val, double grad[], double var[]);
        EMatrixX kernel_sparse_deriv1_3D(EMatrixX const& x1, EMatrixX const& x2, float scale_param);
        void testSinglePointG(const EVectorX& xt, double& val, double grad[],double var[]);
    };
