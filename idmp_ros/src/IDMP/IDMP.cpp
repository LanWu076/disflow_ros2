...
namespace IDMP_ros
{
    ...
    pcl::PointCloud<pcl::PointXYZRGB> IDMP::processFrame(pcl::PointCloud<pcl::PointXYZRGB> &cld, std::vector<Eigen::Matrix4f> &camPoses) {
        ...
        if(t != 0 && (setting.dynamic || setting.fusion)) {
            ...
            if(tempGp.update(cld, camPoses) != 0) {
                for(int i = 0; i<camPoses.size(); i++){
                    if(frustum[i].checkEmpty()){
                        RCLCPP_ERROR(rclcpp::get_logger("idmp"), "Process Frame called before first camera_info callback!");
                        return tempCld;
                    }
                ...
                if(nodes.size() > 0) {
                    ...
                    for(int i = 0; i<nodes.size(); i++){
                        int i8 = i*8;
                        if(setting.dynamic && res[i8]>setting.dyn_tresh) {  //get rid of moved points
                        ...
                        if(setting.fusion && res[i8] > setting.fus_min && res[i8] < setting.fus_max) { //fusion, merge the points in the half way
                            pcl::PointXYZRGB p;
    ...
    int IDMP::update(pcl::PointCloud<pcl::PointXYZRGB> &cld, std::vector<Eigen::Matrix4f> &camPoses) {
        if (t == 0) {
        ...
        // std::cout << "[IDMP update] input cld size = " << cld.points.size()
        //   << ", inserted pointCnt = " << pointCnt << std::endl;
    ...
    void IDMP::updateGPs_kernel(int thread_idx,
        ...
        Eigen::Vector3f camPosOne = camPoses.back().block<3,1>(0,3);

        // std::cout << "camPosOne: "
        //   << camPosOne.x() << ", "
        //   << camPosOne.y() << ", "
        //   << camPosOne.z() << std::endl;
        ...
        for (int i = start_idx; i < end_idx; ++i) {
            if (nodes_to_update[i] != 0) {
                ...
                std::vector<size_t> indices;
                float halfLength = nodes_to_update[i]->getHalfLength();
                float rleng = setting.rleng;
                float radius = halfLength * rleng;
                ...
                if (indices.size() > 0) {
                    ...
                    if (setting.oneshot){
                        ...
                        //ROS_INFO_STREAM("one shot training" << indices.size());
                    }else{
                        // Build a tiny PCL cloud from trainMat
                        pcl::PointCloud<pcl::PointXYZ>::Ptr nbhd(new pcl::PointCloud<pcl::PointXYZ>());
                        nbhd->resize(trainMat.cols());
                        for (int j = 0; j < trainMat.cols(); ++j) {
                            (*nbhd)[j].x = static_cast<float>(trainMat(0,j));
                            (*nbhd)[j].y = static_cast<float>(trainMat(1,j));
                            (*nbhd)[j].z = static_cast<float>(trainMat(2,j));
                        }

                        // normal estimation
                        pcl::NormalEstimation<pcl::PointXYZ, pcl::Normal> ne;
                        ne.setInputCloud(nbhd);
                        //ne.setViewPoint(camPosOne.x(), camPosOne.y(), camPosOne.z()); // auto flip toward camera
                        //auto kdt = boost::make_shared<pcl::search::KdTree<pcl::PointXYZ>>();
                        auto kdt = pcl::make_shared<pcl::search::KdTree<pcl::PointXYZ>>();
                        ne.setSearchMethod(kdt);
                        ne.setKSearch(10);

                        pcl::PointCloud<pcl::Normal>::Ptr nout(new pcl::PointCloud<pcl::Normal>());
                        ne.compute(*nout);

                        std::vector<int> keep_idx;
                        keep_idx.reserve(nbhd->size());

                        Eigen::Matrix<double,3,Eigen::Dynamic> normals_tmp(3, nbhd->size());

                        for (size_t j = 0; j < nbhd->size(); ++j) {
                            Eigen::Vector3f pj((*nbhd)[j].x, (*nbhd)[j].y, (*nbhd)[j].z);
                            Eigen::Vector3d n(nout->at(j).normal_x,
                                            nout->at(j).normal_y,
                                            nout->at(j).normal_z);
                            double nl = n.norm();
                            if (!std::isfinite(nl) || nl < 1e-24) {
                                continue;
                            }
                            n /= nl;                

                            Eigen::Vector3d camd = camPosOne.cast<double>();
                            Eigen::Vector3d pjd = pj.cast<double>();

                            //if (n.dot(camd - pjd) < 0.0) n = -n;

                            normals_tmp.col(static_cast<int>(keep_idx.size())) = n;
                            keep_idx.push_back(static_cast<int>(j));
                        }

                        if (keep_idx.empty()) {
                            std::cout << "[GP TRAIN] skipped all points due to invalid normals." << std::endl;
                        }

                        const int M = static_cast<int>(keep_idx.size());
                        Eigen::Matrix<double,3,Eigen::Dynamic> trainMat_kept(3, M);
                        Eigen::Matrix<double,3,Eigen::Dynamic> normals_kept(3, M);

                        for (int k = 0; k < M; ++k) {
                            int j = keep_idx[k];
                            trainMat_kept.col(k) = trainMat.col(j);          
                            normals_kept.col(k)  = normals_tmp.col(k);      
                        }

                        // std::cout << "[GP TRAIN] kept " << M 
                        //         << " / " << nbhd->size() 
                        //         << " points (invalid normals skipped=" << (nbhd->size() - M) << ")"
                        //         << std::endl;

                        gp->train_new(trainMat_kept, normals_kept);
                    }
    ...
    void IDMP::updateGPs(std::vector<Eigen::Matrix4f> &camPoses) {
        ...
        updateGPs_kernel(0,0,num_elements, nodes_to_update, camPoses);
            ...
            if(nearestCluster != nullptr) { 
                ...
                if (gp != nullptr) {
                    if(setting.oneshot) {
                        ...
                        gp->testSinglePointG(xt, res[k8], &res[k8 + 1], &res[k8 + 4]);
                    }
                }
            }
            if(setting.oneshot) {
    ...
    bool IDMP::test(float *x, int dim, int leng, double *res) {
        if (x == 0 || dim != 3 || leng < 1)
            return false;
        if(t == 0) {
            RCLCPP_ERROR(rclcpp::get_logger("idmp"), "GP IS NOT TRAINED!");
            return false;
        }
